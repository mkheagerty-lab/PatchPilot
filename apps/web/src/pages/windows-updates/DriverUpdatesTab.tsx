    ...
    <div>
      ...
      {isLoading ? (
        <Card className="border-dashed">
          <p className="text-sm text-slate-500">Loading driver update profiles…</p>
        </Card>
      ) : profiles.length === 0 ? (
        <Card className="border-dashed">
        ...
        <Card className="border-dashed">
          <p className="text-sm text-slate-500">No profiles match "{table.search.trim()}".</p>
        </Card>
      ) : (
        <Card className="p-0">
            ...
            <tbody>
              {table.sorted.map((p) => (
                <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-600">{p.approvalType ?? "—"}</td>
