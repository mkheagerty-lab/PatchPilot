  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
        held ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"
      }`}
    >
      {held ? "Held" : "Not held"}
    </span>
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-slate-500">
        ...
        layers, for <span className="font-medium text-slate-700">{activeTenant?.displayName}</span>.
      </p>

      {canCheckOthers && (
        <div className="mb-6 flex items-center gap-2">
          <label className="text-sm font-medium text-slate-700" htmlFor="check-access-user">
            Check access for
          </label>
          <select
            ...
            className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm text-slate-800 focus:border-slate-400 focus:outline-none"
          >
      ...
      <div className="mb-6">
        <h2 className="mb-2 text-sm font-semibold text-slate-700">1. PatchPilot</h2>
          ...
          {isLoading || !displaySummary ? (
            ...
            <>
              <div className="mb-4 flex flex-wrap items-center gap-4">
                ...
                <div>
                  <div className="text-xs text-slate-500">Tenant write-enabled?</div>
                  <div className="text-sm font-medium text-slate-800">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {displaySummary.patchPilot.scopes.map((s) => (
                      <tr key={s.area} className="border-b border-slate-100 last:border-0">
                        <td className="py-2 pr-4 whitespace-nowrap font-medium text-slate-700">
                          {s.label}
                        </td>
                        <td className="py-2 pr-4 text-slate-500">{s.description}</td>
                        <td className="py-2">
      ...
      {displaySummary?.demoMode ? (
        ...
        <Card className="mb-6">
          <p className="text-sm text-slate-500">Checking… this may briefly redirect through Microsoft sign-in.</p>
        </Card>
      ) : null}
      ...
      {liveResult && (
        <>
          <div className="mb-6">
            <h2 className="mb-2 text-sm font-semibold text-slate-700">2. Entra Roles (Direct)</h2>
            <p className="mb-2 text-xs text-slate-500">
            ...
            <Card>
              {!liveResult.entraDirect.applicable ? (
                ...
                <p className="text-sm text-slate-500">No directly assigned roles.</p>
              ) : (
          ...
          <div>
            <h2 className="mb-2 text-sm font-semibold text-slate-700">3. GDAP Roles (Partner Centre)</h2>
            <p className="mb-2 text-xs text-slate-500">
      ...
      <tbody>
        {roles.map((r) => (
          <tr key={r.role} className="border-b border-slate-100 last:border-0">
            <td className="py-2 pr-4 font-medium text-slate-700">{r.role}</td>
            <td className="py-2">
